...
def registrarUsuario(nombre, email, password,codEncript):
    try:
        db = sqlite3.connect("mensajes.s3db")
        db.row_factory = sqlite3.Row
        cursor = db.cursor()
        consulta = "insert into usuarios (nombreusuario,correo,password,estado,codigoactivacion) values('"+nombre+"','"+email+"','"+password+"','0','"+codEncript+"')"
        cursor.execute(consulta)
        db.commit()
